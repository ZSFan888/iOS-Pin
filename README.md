...
- `Worker/Src/Proto/Apple-wloc.ts` 现在按照公开逆向文档确认的真实字段结构改写响应：`[10 字节头部][重复的顶层字段 2 = AccessPoint 条目]`，每个 AccessPoint 条目内字段 1 是 BSSID 字符串，字段 2 是嵌套的 Coordinate 子消息（字段 1 = 纬度、字段 2 = 经度，均为按 1e8 缩放的普通 varint，不是 zigzag）。
- 改写逻辑现在会遍历并重写响应中的**每一个** AccessPoint 条目的坐标（而不仅仅是第一个），因为 locationd 会用多个条目做三角定位，只改第一个会导致最终定位被真实坐标拉偏。BSSID 与其余字段（信号强度、时间戳等）保持原样透传，不影响响应的结构真实性。
- `Worker/Test/Apple-wloc.test.ts`（基于 Vitest）覆盖：单条目坐标替换、BSSID 保持不变、多条目全部改写、南半球/西半球负数坐标的符号正确性、过短响应体的直通逻辑，以及 `decimalToMicro`/`microToDecimal` 的缩放与四舍五入。
- 新增了一条自动生效的真实抓包回归测试：只要 `Worker/Test/Fixtures/sample-01.bin` 存在，`it.skipIf(!hasFixture)` 就会自动从跳过变为执行，无需手动改代码。
