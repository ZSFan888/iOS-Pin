...
import { decimalToMicro, spoofAppleIosPinResponse } from './Apple-ios-pin'
...
function noStoreHeaders(extra?: Record<string, string>) {
  const headers = new Headers({ 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store, no-cache, must-revalidate, max-age=0', pragma: 'no-cache', expires: '0' })
  if (extra) for (const [k, v] of Object.entries(extra)) headers.set(k, v)
  return headers
}
function textNoStoreResponse(body: string, contentType = 'text/plain; charset=utf-8') {
  return new Response(body, { headers: { 'content-type': contentType, 'cache-control': 'no-store, no-cache, must-revalidate, max-age=0', pragma: 'no-cache', expires: '0' } })
}
function normalizeToken(token: string) { return token.trim() }
function isValidToken(token: string) { return /^[a-zA-Z0-9_-]{1,64}$/.test(token) }
function areValidCoordinates(lat: number, lng: number) { return Number.isFinite(lat) && Number.isFinite(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180 }
...
function buildAppleUpstreamUrl(requestedHost?: string | null) { return `https://${normalizeAppleHost(requestedHost)}${APPLE_NETWORK_LOCATION_PATH}` }
...
async function requireWriteAuth(c: any, next: () => Promise<void>) {
  ...
  if (token && !isTokenAllowed(env, token)) return c.json({ error: 'token not allowed' }, 403)
  if (env.API_KEY) {
    const provided = c.req.header('x-ios-pin-key')
    if (provided !== env.API_KEY) return c.json({ error: 'unauthorized' }, 401)
  }
  await next()
}
...
app.post('/api/history/:token', requireWriteAuth, async (c) => {
  ...
  const raw = await c.env.LOCATIONS.get(`history:${token}`, 'json') as HistoryEntry[] | null
  const items = raw ?? []
  const entry: HistoryEntry = { label: body.label || `${body.lat.toFixed(4)}, ${body.lng.toFixed(4)}`, lat: body.lat, lng: body.lng, savedAt: new Date().toISOString() }
...
export const onRequest: PagesFunction<Bindings> = async (context) => app.fetch(context.request, context.env, context)
